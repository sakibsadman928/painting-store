...
const purchaseTickets = async (req, res) => {
  try {
    ...
    if (exhibition.status !== "active") {
      return res.json({
        success: false,
        message: "Exhibition is not available for booking",
      });
    }
    ...
    await Exhibition.findByIdAndUpdate(exhibitionId, {
      ...
      $set: {
        status:
          exhibition.availableTickets - quantity === 0 ? "sold-out" : "active",
      },
    });

    const populatedTicket = await Ticket.findById(ticket._id).populate(
      "exhibitionId",
      "title eventDate eventTime venue",
    );
...
const getAllExhibitions = async (req, res) => {
  try {
    ...
    const exhibitions = await exhibitionService.getExhibitionsByMonth(
      month,
      year,
    );
    res.json({ success: true, exhibitions });
