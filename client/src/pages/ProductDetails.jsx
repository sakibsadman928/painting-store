...
const ProductDetails = () => {
  ...
  const { navigate, updateCartItem, cartItems, user, setShowUserLogin } =
    useAppContext();
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.product.image && data.product.image.length > 0) {
          setSelectedImage(`${API_URL}/images/${data.product.image[0]}`);
  ...
  const fetchProductRatings = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/rating/get`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleImageSelect = (imageName, index) => {
    setSelectedImage(`${API_URL}/images/${imageName}`);
    setSelectedImageIndex(index);
  };

  const openFullScreenImage = (imageName, index) => {
    setFullScreenImage(`${API_URL}/images/${imageName}`);
  ...
  const navigateFullScreenImage = (direction) => {
    ...
    setFullScreenImage(`${API_URL}/images/${product.image[newIndex]}`);
  };
            ...
            <div className="flex-1 mx-2">
              <div className="bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${ratings.length > 0 ? (distribution[star - 1] / ratings.length) * 100 : 0}%`,
                  }}
                ></div>
              </div>
            </div>
        ...
        <div className="lg:w-1/2">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="text-gray-600">
              <span className="font-medium">{product.totalRatings || 0}</span>{" "}
              review{product.totalRatings !== 1 ? "s" : ""}
            </div>
          </div>
            ...
            {product.stock > 0 ? (
              <div>
                ...
                {user && currentQuantity > 0 && (
                  <p className="text-[#bb86fc] font-medium">
          ...
          {product.stock > 0 && (
            <div className="mb-8">
              <label className="block text-sm font-medium text-gray-700 mb-3">
                Quantity{" "}
                {!user && <span className="text-red-500">*Login required</span>}
              </label>
              <div className="flex items-center gap-4">
                <div className="flex items-center border border-gray-300 rounded">
                  ...
                  >
                    {Array.from(
                      { length: Math.min(product.stock + 1, 11) },
                      (_, i) => (
            ...
            <div className="mb-8 p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center text-green-800">
                ...
                <span className="font-medium">
                  {currentQuantity} item{currentQuantity > 1 ? "s" : ""} added
                  to cart
                </span>
              </div>
            </div>
