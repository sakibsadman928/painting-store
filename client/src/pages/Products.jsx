    ...
    async (params = {}) => {
      try {
        ...
        const response = await fetch(
          `${API_URL}/api/product/list?${queryParams}`,
        );
      ...
      return (
        <div className="col-span-full text-center py-8">
          <p className="text-gray-500 mb-4">
            {searchTerm
              ? `No paintings found for "${searchTerm}"`
  ...
  return (
    <div className="mt-16 px-6 md:px-16 lg:px-24 xl:px-32 mb-16">
      <div className="flex flex-col items-center w-full mb-8">
        <h1 className="text-2xl font-medium uppercase text-[#6b46c1]">
          All Paintings
        </h1>
      ...
      {!loading && !error && products.length > 0 && (
        <div className="mt-8">
          <div className="text-center text-gray-600 mb-4">
            ...
            {Math.min(currentPage * 20, pagination.totalProducts || 0)} of{" "}
            {pagination.totalProducts || 0} paintings
          </div>
          {renderPagination()}
        </div>
      )}
